...
def exp6_run_forone(path, sub, sub_base):
    ...
    if not os.path.exists(f'../save_folder_exp6/{sub}_100percent'):
        os.makedirs(f'../save_folder_exp6/{sub}_100percent')
    ...
    save_path_1 = f'../save_folder_exp6/{sub}_100percent/{sub}_model_oh_{sub_base}.pt'
    metric = Accuracy()
    ...
    # Record the results.
    file_path = f"../save_folder_exp6/{sub}_100percent/best_score_log.txt"
    ...
    file.write(f"{sub}_best_score_{sub_base}: {max(best_score_log)} \n")
    file.close()
