...
def exp5_run_forone_2(path, sub):
    ...
    if not os.path.exists(f'../save_folder_exp5/average_represent'):
        os.makedirs(f'../save_folder_exp5/average_represent')
    if not os.path.exists(f'../save_folder_exp5/average_represent/{sub}'):
        os.makedirs(f'../save_folder_exp5/average_represent/{sub}')
    ...
    pre_trainloader = DataLoader(pre_trainset, shuffle=True, batch_size=32, drop_last=False, collate_fn=collate_fn1)
    ...
    save_path_0 = f'../save_folder_exp5/average_represent/{sub}/tc_encoder_{sub}.pt'
    ...
    runner0.train(train_loader=pre_trainloader, num_epochs=16, log_stride=8)
    ...
    save_path_1 = f'../save_folder_exp5/average_represent/{sub}/model_oh_{sub}.pt'
    metric = Accuracy()
    ...
    # save_path_2 = f'../save_folder_exp5/average_represent/{sub}/model_oh_{sub}.pt'
    # metric = Accuracy()
    # loss = nn.CrossEntropyLoss()
    # opt = optim.SGD(model_oh_finetune1.parameters(), lr=0.00001)
    # runner2 = Runner_v5_3(model=model_oh_finetune1, loss_fn=loss, optimizer=opt, metric=metric, save_path=save_path_2)
    # runner2.best_score=runner1.best_score
    # print(f'The finetune 1 of {sub} starts!')
    # runner2.train(train_loader=trainloader, dev_loader=devloader, num_epochs=128, log_stride=32)
...
def exp5_run_forall_2(path, all_sub):
    best_scores = []
    for sub in all_sub:
        try:
...
exp5_run_forall_2(DB1a_path,DB1a_subs)
exp5_run_forall_2(DB1b_path, DB1b_subs)
