...
def run_for_one(path, sub):
    ...
    transformer_oh = Transformer_OH2(linear_project, encoder, decoder, embedding, classifier).to(device)
    metric = Accuracy()
    loss_fn = nn.CrossEntropyLoss()
    opt = Adam(transformer_oh.parameters(), lr=0.0001)
    runner = Runner_v5_3(model=transformer_oh, loss_fn=loss_fn, optimizer=opt, metric=metric)
