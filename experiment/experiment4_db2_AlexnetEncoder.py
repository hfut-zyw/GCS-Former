...
def exp1_run_forone(sub):
    ...
    # The pre-training
    encoder = AlexNet_exp4_2(input_dim * channels).to('cuda')
    linear_project = Linear_Project().to('cuda')
    model_pretrain = Model_pretrain(encoder, linear_project)
    save_path_1 = f'../save_folder_exp4/db2/Alexnet_encoder_{sub}.pt'
    gesture_contrastive_loss = Gesture_Contrastive_Loss(temperature=0.02)  # 0.01 performs better
    ...
    runner_1.train(train_loader=trainloader_1, num_epochs=4, log_stride=64)
    runner_1.load(save_path_1)

    # The formal training
    best_score_log = []
    encoder = model_pretrain.encoder
    decoder = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=512, nhead=1, batch_first=True), 1)
    embedding = nn.Embedding(4, 512)
    classifier = Classifier(num_classes=num_classes)
    model = GCS_Former(encoder, decoder, embedding, classifier).to('cuda')
    save_path_2 = f'../save_folder_exp4/db2/model_Alexnet_encoder_{sub}.pt'
    metric = Accuracy2()
    ...
    runner.train(train_loader=trainloader_2, dev_loader=devloader_2, num_epochs=1, log_stride=128)
    ...
    save_path_3 = f'../save_folder_exp4/db2/model_Alexnet_encoder_finetune1_{sub}.pt'
    metric_finetune = Accuracy2()
    loss_finetune = nn.CrossEntropyLoss()
    opt_finetune1 = optim.Adam(model_finetune1.parameters(), lr=0.0001)
    ...
    runner_finetune1.train(train_loader=trainloader_2, dev_loader=devloader_2, num_epochs=8, log_stride=128)
    ...
    save_path_4 = f'../save_folder_exp4/db2/model_Alexnet_encoder_finetune2_{sub}.pt'
    ...
    runner_finetune2.train(train_loader=trainloader_2, dev_loader=devloader_2, num_epochs=2, log_stride=128)
    ...
    save_path_5 = f'../save_folder_exp4/db2/model_Alexnet_encoder_finetune3_{sub}.pt'
    ...
    runner_finetune3.train(train_loader=trainloader_2, dev_loader=devloader_2, num_epochs=1, log_stride=128)
    best_score_log.append(runner_finetune3.best_score)

    # Record the results.
    file = open("../save_folder_exp4/db2/best_score_Alexnet_encoder.txt", "a")
    file.write(f"{sub}_best_score: {max(best_score_log)} \n")
    file.close()
    print(f'model_AlexEncoder of {sub} is finished!')
    print('\n')
...
def exp1_run_forall(all_sub):
    ...
    for sub in all_sub:
        ...
        # except:
        # print(f"{sub} wrong! Please check cuda memory!")
    print("all done!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print(best_scores)
