...
def run_for_one(path, sub):
    ...
    trainloader = DataLoader(trainset, shuffle=True, batch_size=batch_size_train, collate_fn=seqmodels_collate_fn2,
                             drop_last=True)
    devloader = DataLoader(testset, shuffle=False, batch_size=batch_size_test, collate_fn=seqmodels_collate_fn2,
                           drop_last=False)

    linear_project = nn.Linear(segment_D, embedding_dim).to(device)

    encoder = TransformerEncoder(nn.TransformerEncoderLayer(d_model=embedding_dim, nhead=8, batch_first=True), 3).to(
        device)
    decoder = nn.TransformerDecoder(nn.TransformerDecoderLayer(d_model=embedding_dim, nhead=8, batch_first=True), 2)

    embedding = nn.Embedding(1, embedding_dim)
    classifier = Classifier(embedding_dim, num_classes).to(device)
    transformer_oh = Transformer_OH(linear_project, encoder, decoder, embedding, classifier).to(device)
    metric = Accuracy()
    loss_fn = nn.CrossEntropyLoss()
    opt = Adam(transformer_oh.parameters(), lr=0.0001)
    runner = Runner_v5_2(model=transformer_oh, loss_fn=loss_fn, optimizer=opt, metric=metric)
