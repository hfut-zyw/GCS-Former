...
def exp5_run_forone_2(path, sub):
    ...
    resnet18 = ResNet18(encoder, classifier).to(device)

    metric = Accuracy()
    loss = nn.CrossEntropyLoss()
    opt = optim.Adam(resnet18.parameters(), lr=0.001)
    runner = Runner_v5_3(model=resnet18, loss_fn=loss, optimizer=opt, metric=metric)
