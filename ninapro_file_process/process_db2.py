...
def process_dataset_for_oneSubject(_subject):
    data_paths = sorted(glob.glob(f'../data/ninapro/db2/{_subject}/*.mat'))
    ...
    if not os.path.exists('../data/ninapro/db2_processed/{}'.format(_subject)):
        os.makedirs('../data/ninapro/db2_processed/{}'.format(_subject))
    np.savetxt('../data/ninapro/db2_processed/{}/emg.txt'.format(_subject), emg)
    np.savetxt('../data/ninapro/db2_processed/{}/restimulus.txt'.format(_subject), restimulus, fmt="%d")
    np.savetxt('../data/ninapro/db2_processed/{}/rerepetition.txt'.format(_subject), rerepetition, fmt="%d")
    np.savetxt('../data/ninapro/db2_processed/{}/exercise.txt'.format(_subject), exercise, fmt="%d")


_subjects = sorted(os.listdir('../data/ninapro/db2/'))
print(_subjects)
